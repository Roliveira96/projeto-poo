...
export class TelaMenu implements Tela {
  ...
  public montar(raiz: HTMLElement): void {
    ...
    raiz.innerHTML =
      ...
      '    <button class="botao-secundario menu-materiais" data-acao="materiais">📚 Materiais: história e referências</button>' +
      '  </header>' +
      ...
      'Em cada demo, os controles ⏮ ▶ ⏭ (ou as teclas ← espaço →) deixam você avançar no seu ritmo.</p>' +
      '</div>';

    this.modal = new JanelaModal();
    (raiz.querySelector('[data-acao="materiais"]') as HTMLElement).addEventListener('click', () => {
      this.modal?.abrir('📚 Materiais: a história dos padrões de projeto', new MaterialDeApoio().gerarHtml(''));
    });
  }
